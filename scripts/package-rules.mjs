...
export function prohibitedArchiveReason(value) {
  ...
  if (/(^|\/)(dist|coverage|\.vite|\.cache|playwright-report|test-results|screenshots|\.artifacts)(?:\/|$)/i.test(file)) return "generated build/test output";
...
export function sensitiveTextReason(text) {
  ...
  if (/[A-Za-z]:\\Users\\[^\s"']+|(?:^|[\s"'(=])\/(?:Users|home)\/[^/\s"']+/im.test(text)) return "absolute local user path";
  for (const line of text.split(/\r?\n/)) {
    const match = line.match(/\b(JWT_SECRET|BOOTSTRAP_ADMIN_PASSWORD|DB_PASSWORD)\b\s*[:=]\s*["']?([^\s,"'}]+)/i);
    if (!match) continue;
    const value = match[2];
    if (/\$\{|\$[A-Z_]|process\.env|replace|change|example|sample|test|smoke|e2e|capture|ci-only|regression|fake|fixture|development|combined-group|rootpass|required|^$/i.test(value)) continue;
...
export function assertRuleSelfTests() {
  const prohibited = [
    ".git/config", "server/.env", "client/node_modules/x", "client/dist/app.js", ".artifacts/task/failure.png",
  ...
  const sensitive = [
    ...
    "C:\\Users\\person\\secret.txt", "/home/person/secret.txt",
  ];
  ...
  if (sensitiveTextReason("JWT_SECRET=regression-only-secret-with-at-least-32-characters"))
    throw new Error("Package-rule self-test rejected an explicit regression secret");
  if (sensitiveTextReason("https://pixabay.com/users/teacher-7/"))
    throw new Error("Package-rule self-test mistook a URL path for a local user path");
  for (const source of [
