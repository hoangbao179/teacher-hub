...
test("template renders four sheets, parent-safe absence and protected technical metadata", () => {
  const plan = new GoogleSheetTemplateService().build(snapshot, "sheet-1",
    { "Tổng quan": 1, "Nhật ký học tập": 2, "Học phí": 3, _TeacherHub: 4 },
    { templateVersion: "v1", generatedAt: "2026-07-26T00:00:00Z" });
  ...
  assert.ok(plan.requests.some((item) => item.addProtectedRange?.protectedRange?.range?.sheetId === 4));
