...
export class ScheduleRepository {
  ...
  async resolveOccurrence(key: string): Promise<ScheduleOccurrence | null> {
    ...
    const [rows] = await pool.query<RowDataPacket[]>(
      `SELECT rs.id recurring_schedule_id,rs.class_id,c.name class_name,c.status,
       ...
       WHERE rs.id=? AND rs.class_id=?`, [parsed.recurringScheduleId, parsed.classId],
    );
    const row = rows[0];
    if (!row || row.status !== "ACTIVE" || Number(row.day_of_week) !== weekdayIso(parsed.occurrenceDate) ||
  ...
  async createException(
    ...
    try {
      ...
      const [schedules] = await connection.query<RowDataPacket[]>(
        `SELECT rs.*,c.status FROM recurring_schedules rs JOIN classes c ON c.id=rs.class_id
         WHERE rs.id=? AND rs.class_id=? FOR UPDATE`, [parsed.recurringScheduleId, parsed.classId],
      );
      const schedule = schedules[0];
      if (!schedule || schedule.status !== "ACTIVE" || Number(schedule.day_of_week) !== weekdayIso(parsed.occurrenceDate) ||
  ...
  async detectConflicts(date: string, startTime: string, endTime: string, excludeOriginalKey?: string): Promise<ScheduleConflictWarning[]> {
    ...
    for (const item of lessons) {
      if (item.status === "CANCELLED" || (excludeOriginalKey != null &&
  ...
  private async listOccurrencesCore(from: string, to: string, classId?: number, includeConflicts = false): Promise<ScheduleOccurrence[]> {
    ...
    const [scheduleRows] = await pool.query<RowDataPacket[]>(
      `SELECT rs.id recurring_schedule_id,rs.class_id,c.name class_name,rs.day_of_week,
        ...
        TIME_FORMAT(rs.start_time,'%H:%i') start_time,TIME_FORMAT(rs.end_time,'%H:%i') end_time
       FROM recurring_schedules rs JOIN classes c ON c.id=rs.class_id
       WHERE c.status='ACTIVE' AND rs.effective_from<=? AND (rs.effective_to IS NULL OR rs.effective_to>=?)${scheduleClassFilter}`,
      scheduleParams,
    );
    ...
    const projected = expandRecurringSchedules(scheduleRows.map(mapSchedule), expansionFrom, expansionTo);
  ...
  private async listLessonEvents(from: string, to: string, classId?: number): Promise<LessonEvent[]> {
    ...
    if (classId) params.push(classId);
    const [rows] = await pool.query<RowDataPacket[]>(
      `SELECT l.id,l.source_occurrence_key,l.class_id,c.name class_name,
...
function mapSchedule(row: RowDataPacket): RecurringProjectionInput {
  return { recurringScheduleId: Number(row.recurring_schedule_id), classId: Number(row.class_id), className: String(row.class_name),
    ...
    effectiveFrom: String(row.effective_from), effectiveTo: row.effective_to == null ? null : String(row.effective_to) };
}
