...
test("Homepage has one bootstrap JSON-LD script and an optional Maps key", () => {
  assert.equal((indexHtml.match(/id="public-home-structured-data"/g) ?? []).length, 1);
  assert.match(envExample, /# Optional\. Homepage falls back to a local location card when omitted\.\r?\nVITE_GOOGLE_MAPS_EMBED_API_KEY=/);
});
