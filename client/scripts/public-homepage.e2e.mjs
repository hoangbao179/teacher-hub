...
const artifactDir = path.join(root, ".agent-reports", "v1-1-after-branding");
...
try {
  ...
  for (const heading of ["Đồng hành theo năng lực", "Tiếng Anh lớp 1–9", "Rõ ràng, vừa sức", "Xem thử cách", "Mẫu bố cục phản hồi", "Trao đổi về mục tiêu"]) {
  ...
  if (metadata.title !== "Lớp học tiếng Anh cô Vy | Tiếng Anh lớp 1–9" || !metadata.description?.includes("Nguyễn Tri Phương") || metadata.ogTitle !== metadata.title) throw new Error("Cô Vy Homepage metadata is incomplete or inconsistent");
  ...
  for (const viewport of [{ width: 360, height: 800 }, { width: 375, height: 812 }, { width: 390, height: 844 }, { width: 393, height: 852 }, { width: 412, height: 915 }, { width: 430, height: 932 }]) {
    ...
    await page.screenshot({ path: path.join(artifactDir, `public-home-${viewport.width}x${viewport.height}.png`), fullPage: true });
  }
  ...
  const screenshot = path.join(artifactDir, "public-home-1440x900.png");
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.goto(origin);
  await page.getByRole("heading", { level: 1, name: /Tiếng Anh vững nền tảng/ }).waitFor();
  await page.screenshot({ path: screenshot, fullPage: true });
  console.log(`V11A Homepage E2E passed at 360–430px with Cô Vy branding; screenshots: ${artifactDir}`);
} finally {
