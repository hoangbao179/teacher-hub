...
export function VocabularyImagePicker({ open, word, meaningVi, searchTerms = [], onClose, onSelect, onSelectLocal }: {
  ...
  const [currentProvider, setCurrentProvider] = useState<"ARASAAC" | "PIXABAY" | "">("");
  ...
  useEffect(() => {
    ...
    void getVocabularyMediaStatus(controller.signal)
      .then((value) => {
        ...
        const pixabayEnabled = value.providers.some((provider) =>
          provider.provider === "PIXABAY" && provider.enabled);
        setPhotoEnabled(pixabayEnabled);
        if (!pixabayEnabled) setMediaType("ILLUSTRATION");
      })
  ...
  const search = useCallback(async (targetPage = 1) => {
    ...
    try {
      ...
      setCurrentProvider(result.provider === "ARASAAC" || result.provider === "PIXABAY"
        ? result.provider
        : "");
      setDisabled(false);
    } catch (value) {
      ...
      <DialogContent dividers>
        <Stack spacing={1.5} sx={{ minWidth: 0 }}>
          {strategy.publicAsset ? <>
            <Alert severity="info">Hình chuẩn có sẵn, không cần tìm từ nguồn bên ngoài.</Alert>
            ...
            {disabled && <Alert severity="info">Nguồn hình minh họa đang tắt. Bạn vẫn có thể tải ảnh từ máy.</Alert>}
            ...
            {items.length > 0 && <>
              <Stack direction="row" sx={{ justifyContent: "space-between", alignItems: "center", gap: 1 }}><Typography variant="caption" color="text.secondary">Nguồn {currentProvider || "hình ảnh"} · Ảnh chỉ được lưu sau khi cô chọn.</Typography><Typography variant="caption" sx={{ fontWeight: 700 }}>{items.length}/{VOCABULARY_IMAGE_LIMIT} ảnh</Typography></Stack>
              <Box sx={{ display: "grid", gridTemplateColumns: { xs: "repeat(2, minmax(0, 1fr))", sm: "repeat(4, minmax(0, 1fr))", md: "repeat(5, minmax(0, 1fr))" }, gap: 1, minWidth: 0 }}>
                {items.map((item) => {
                  ...
                  <Box component="img" src={item.thumbnailUrl} alt={`${word} — ${meaningVi}`} sx={{ width: "100%", aspectRatio: "1", objectFit: item.provider === "ARASAAC" ? "contain" : "cover", bgcolor: item.provider === "ARASAAC" ? "grey.50" : undefined, display: "block" }} />
                  <Stack direction="row" sx={{ px: 0.75, py: 0.5, minHeight: 44, gap: 0.5, alignItems: "center", justifyContent: "center" }}>{importing === item.providerAssetId && <CircularProgress size={14} />}<Typography variant="caption">{selected ? "Đã chọn" : "Chọn"}</Typography>{selected && <CheckCircle fontSize="small" color="primary" />}</Stack>
                </Button>;
                })}
              </Box>
            ...
            {items.length > 0 && currentProvider === "ARASAAC" && <Typography variant="caption" color="text.secondary">Pictogram: Sergio Palao / ARASAAC · Government of Aragón · CC BY-NC-SA.</Typography>}
          </>}
