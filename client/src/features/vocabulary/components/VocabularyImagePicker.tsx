...
export function VocabularyImagePicker({ open, word, meaningVi, searchTerms = [], onClose, onSelect }: {
  open: boolean;
  word: string;
  meaningVi: string;
  searchTerms?: string[];
  ...
  const search = useCallback(async () => {
    ...
    setError("");
    try {
      const result = await searchVocabularyMedia({ query, mediaType, pageSize: 20 });
  ...
  return (
    <Dialog open={open} onClose={onClose} fullScreen={fullScreen} fullWidth maxWidth="md" data-testid="vocabulary-image-picker">
      ...
      <DialogContent dividers>
        <Stack spacing={1.5}>
          <Box component="form" onSubmit={(event) => { event.preventDefault(); void search(); }} sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "minmax(0, 1fr) 180px auto" }, gap: 1 }}>
            <TextField
              autoFocus
              label="Từ khóa tìm ảnh"
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              slotProps={{ input: { startAdornment: <InputAdornment position="start"><Search /></InputAdornment> }, htmlInput: { maxLength: 100 } }}
            />
            <TextField select label="Loại ảnh" value={mediaType} onChange={(event) => setMediaType(event.target.value as VocabularyImageMediaType)}>
          ...
          {!loading && !disabled && !error && items.length === 0 && <Box sx={{ textAlign: "center", py: 5 }}><ImageSearch color="disabled" sx={{ fontSize: 48 }} /><Typography color="text.secondary">Nhập từ khóa rồi bấm Tìm để xem ảnh an toàn cho trẻ em.</Typography></Box>}
          {!loading && items.length > 0 && <>
            ...
            <Box sx={{ display: "grid", gridTemplateColumns: { xs: "repeat(2, minmax(0, 1fr))", sm: "repeat(4, minmax(0, 1fr))" }, gap: 1 }}>
              {items.map((item) => <Button
                key={item.providerAssetId}
                onClick={() => void choose(item)}
                disabled={Boolean(importing)}
                aria-label={`Chọn ảnh của ${item.contributorName}`}
                sx={{ p: 0, minWidth: 0, display: "block", overflow: "hidden", border: 1, borderColor: "divider", borderRadius: 2, textTransform: "none", color: "text.primary" }}
              >
                <Box component="img" src={item.thumbnailUrl} alt={`${word} — ${meaningVi}`} sx={{ width: "100%", aspectRatio: "1", objectFit: "cover", display: "block" }} />
                <Stack direction="row" sx={{ p: 0.75, gap: 0.5, alignItems: "center" }}>
                  {importing === item.providerAssetId && <CircularProgress size={14} />}
                  <Typography variant="caption" noWrap>{item.contributorName}</Typography>
                  {importing === item.providerAssetId && <CheckCircle fontSize="small" color="primary" />}
                </Stack>
              </Button>)}
            </Box>
          </>}
