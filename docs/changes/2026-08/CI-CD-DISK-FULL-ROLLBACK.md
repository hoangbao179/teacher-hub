...
`DB_PASSWORD`, `MYSQL_ROOT_PASSWORD` và `GHCR_OWNER`. Mỗi deploy pull tag full SHA mới,
nhưng cleanup cũ chỉ prune dangling image nên các image SHA có tag có thể tích lũy.
...
- Trước pull, chỉ giữ application SHA đang active. Sau readiness, giữ SHA mới và đúng
  một SHA rollback; cleanup giới hạn theo repository API/Web và không dùng force.
- Maintenance lỗi sau readiness phải báo CI bằng exit code riêng, không rollback một
  application đã healthy.
...
- Bổ sung local image retention hai pha để dọn SHA cũ trước pull và chặn tái tích lũy
  sau deploy.

## Verification

- `bash scripts/deploy-env.test.sh` kiểm tra atomic tag update và retention chỉ chọn
  obsolete full-SHA tag trong đúng repository.
...
nếu không, lỗi cũ có thể tái diễn. Image SHA local đã prune có thể pull lại từ GHCR bằng
immutable tag khi package version tương ứng vẫn còn trong registry.
...
Retention local không xóa các package version cũ trên GHCR; chúng không chiếm SSD VPS và
cần policy registry riêng nếu chi phí/lưu trữ GHCR trở thành vấn đề.
